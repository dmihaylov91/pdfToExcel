...
public class GeneraliGoPdfReader extends AbstractPdfTextReader{

	public GeneraliGoPdfReader() {
		super();
		...
		REGISTRATION_NUMBER_SEARCH_STRING_END = "Категория МПС".replaceAll("\\s+", "");
		
		VEHICLE_BRAND_SEARCH_STRING = "Марка/Make".replaceAll("\\s+", "");
		VEHICLE_BRAND_SEARCH_STRING_END = "Цвят/Color".replaceAll("\\s+", "");
	...
	protected void readPdf(String pdf, SmsSystemDataRow row) {
		...
		row.setInsuranceCompany("Generali-go");
